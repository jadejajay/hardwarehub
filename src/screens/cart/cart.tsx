...
class Cart1 extends React.Component {
  ...
  selectHandler = (index: string | number, value: number) => {
  ...
  quantityHandler = (action: string, index: string | number) => {
    const newItems = [...this.state.cartItems]; // clone the array
  ...
  subtotalPrice = () => {
    ...
    if (cartItems) {
      return cartItems.reduce(
        (
          sum: number,
          item: { checked: number; qty: number; salePrice: number }
          ...
          >
            ...
            >
              <TouchableOpacity
                ...
                onPress={() => {
                  console.log(navigation.navigate('CardDetails'));
                }}
              >
...
export function Cart(props: any) {
  ...
  return <Cart1 {...props} navigation={navigation} />;
}
